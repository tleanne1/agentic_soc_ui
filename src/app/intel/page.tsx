...
function sevBadge(sev: string) {
  const label = safe(sev).toUpperCase();
  return (
    <span className="inline-flex items-center rounded border border-slate-700 bg-slate-200/10 px-2 py-0.5 text-[11px] text-slate-200">
      {label}
    </span>
...
export default function IntelPage() {
  ...
  const decision = useMemo(() => {
    if (!index) return null;
    return decideSocActions({ index, killchain: commandCenter });
  }, [index, commandCenter]);
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded border border-slate-800 bg-[#050A14] p-3">
                ...
                <div className="text-sm text-slate-200 mt-1 flex flex-wrap gap-1">
                  {(commandCenter?.stages?.length ? commandCenter.stages : ["(none)"]).map((s: any, i: number) => (
                    <span key={`stage-${i}-${String(s)}`}>{pill(String(s))}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded border border-slate-800 bg-[#050A14] p-3">
                ...
                <div className="text-sm text-slate-200 mt-1 flex flex-wrap gap-1">
                  {(commandCenter?.next_likely?.length ? commandCenter.next_likely : ["(none)"]).map(
                    (s: any, i: number) => <span key={`next-${i}-${String(s)}`}>{pill(String(s))}</span>
                  )}
                </div>
              </div>
              ...
              <div className="rounded border border-slate-800 bg-[#050A14] p-3">
                ...
                <div className="text-sm text-slate-200 mt-1 flex flex-wrap gap-1">
                  {(commandCenter?.evidence?.mitre_techniques?.length
                    ...
                    <span key={`mitre-${i}-${String(t)}`}>{pill(String(t))}</span>
                  ))}
                </div>
            ...
            <div className="rounded border border-slate-800 bg-[#050A14] p-3">
              ...
              <ul className="mt-2 space-y-1 text-sm text-slate-200 list-disc pl-5">
                {(commandCenter?.evidence?.signals?.length
                  ...
                  <li key={`sig-${i}-${String(s).slice(0, 12)}`}>{String(s)}</li>
                ))}
              </ul>
            </div>
          </div>
            ...
            <div className="rounded border border-[var(--soc-panel-border)] bg-[#020617]/80 p-4 space-y-3">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-xs text-slate-300">
                  Inputs: {decision.inputs_used.join(" + ")}
                </div>
              </div>

              <div className="space-y-3">
                {decision.recommendations.map((r, idx) => (
                  <div key={`rec-${r.id}-${idx}`} className="rounded border border-slate-800 bg-[#050A14] p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="text-sm text-slate-200 font-semibold">{r.title}</div>
                      {sevBadge(r.severity)}
                    </div>

                    <div className="text-xs text-slate-400 mt-2">Rationale</div>
                    <ul className="mt-1 space-y-1 text-sm text-slate-200 list-disc pl-5">
                      {r.rationale.map((x, i) => (
                        <li key={`rat-${r.id}-${i}`}>{x}</li>
                      ))}
                    </ul>

                    <div className="text-xs text-slate-400 mt-3">Suggested analyst actions</div>
                    <ul className="mt-1 space-y-1 text-sm text-slate-200 list-disc pl-5">
                      {r.suggested_actions.map((x, i) => (
                        <li key={`act-${r.id}-${i}`}>{x}</li>
                      ))}
                    </ul>

                    <div className="text-xs text-slate-400 mt-3">Suggested hunts</div>
                    <ul className="mt-1 space-y-1 text-sm text-slate-200 list-disc pl-5">
                      {r.suggested_hunts.map((x, i) => (
                        <li key={`hunt-${r.id}-${i}`}>{x}</li>
                      ))}
                    </ul>

                    <div className="text-xs text-slate-400 mt-3">Guardrails</div>
                ...
                {hits.length ? (
                  <div className="divide-y divide-slate-800">
                    {hits.slice(0, 12).map((h: any, i: number) => {
                      ...
                      return (
                        <div key={`hit-${type}-${i}`} className="bg-[#020617]/40 p-3 flex items-center justify-between gap-4">
                          <div>
                            <div className="text-sm text-slate-200">
